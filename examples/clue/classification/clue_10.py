...
class Clue(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.name in {"chid", "c3"}:
            ...
            if self.config.name in {"tnews", "cluewsc2020"}:
                test_file = "test1.0.json"
            else:
                test_file = "test.json"
    ...
    def _generate_examples(self, data_file, split):
        ...
        if self.config.name == "chid" and split != "test":
            answer_file = os.path.join(
                os.path.dirname(data_file), f"{split}_answer.json"
            )
        ...
        if self.config.name == "c3":
            if split == "test":
                ...
                files = [
                    os.path.join(data_dir, f"{typ}-{split}.json") for typ in ["d", "m"]
                ]
            data = []
            for f in files:
            ...
            with open(data_file, encoding="utf8") as f:
                if self.config.name in ["cmrc2018", "drcd"]:
                    ...
                    for n, line in enumerate(f):
                        ...
                        ):  # CHID has a separate gold label file
                        ...
                        if self.config.name == "cluewsc2020":
                            text = example["text"]
                            s1 = example["target"]["span1_index"]
                            e1 = s1 + len(example["target"]["span1_text"])
                            s2 = example["target"]["span2_index"]
                            e2 = s2 + len(example["target"]["span2_text"])
                            if s1 < s2:
                                text = (
                                    text[:s1]
                                    + "_"
