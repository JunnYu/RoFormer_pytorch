...
""" Finetuning a 🤗 Transformers model for sequence classification on CLUE."""
import argparse
import json
import logging
import math
...
def parse_args():
    ...
    parser.add_argument(
        "--max_grad_norm", default=None, type=float, help="Max gradient norm."
    )
    parser.add_argument(
        "--logging_steps",
        type=int,
...
def main():
    ...
    accelerator = (
        Accelerator(log_with="tensorboard", logging_dir=args.output_dir)
    ...
    if accelerator.is_local_main_process:
        if args.output_dir is not None:
            ...
            logger.addHandler(
                logging.FileHandler(
                    os.path.join(args.output_dir, "training.log"), "w", encoding="utf-8"
                )
            )
    ...
    # or specify a CLUE benchmark task (the dataset will be downloaded automatically from the datasets Hub).
    ...
    if args.task_name is not None:
        ...
        raw_datasets = load_dataset(
            "clue_11.py", args.task_name, cache_dir="./clue_caches"
        )
    else:
    ...
    def preprocess_function(examples):
        ...
        if "label" in examples:
            if label_to_id is not None:
                # Map labels to IDs (not necessary for CLUE tasks)
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("clue_no_trainer", experiment_config)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (
                ...
                if args.logging_steps > 0 and completed_steps % args.logging_steps == 0:
                    logger.info(
                        "completed_steps {} - loss: {:.8f}".format(
                            completed_steps,
                            loss.item(),
                        )
                    )
        ...
        all_metric.append(eval_metric["accuracy"])
        ...
        if eval_metric["accuracy"] >= max_metric:
            max_metric = eval_metric["accuracy"]
            output_dir = "epoch_best"
            if args.output_dir is not None:
                output_dir = os.path.join(args.output_dir, output_dir)
                accelerator.wait_for_everyone()
    ...
    if args.output_dir is not None:
        with open(
            os.path.join(args.output_dir, "all_results.json"), "w", encoding="utf-8"
        ) as f:
            json.dump(
                {"eval_max_accuracy": max(all_metric), "all_metric": all_metric}, f
            )
